...
namespace PSCredentialManager.Cmdlet
{
	...
	[OutputType(typeof(PasswordCredential), ParameterSetName = new[]{"CredentialObject Output"})]
	public class GetStoredCredential : PSCmdlet
	{
		...
		protected override void ProcessRecord()
		{
				...
				try
				{
					...
					if (!AsCredentialObject)
					{
						...
						foreach (var CurrentCredential in Credentials)
						{
							...
							try
							{
								PSCredential psCredential = CurrentCredential.ToPsCredential();
			...
			// If a target is specified, locate only that credential
			else
			{
				try
				{
					...
					var Credential = Vault.FindAllByResource(Target).SingleOrDefault();
	...
	[OutputType(typeof(PasswordCredential))]
	public class NewStoredCredential : PSCmdlet
	{
		...
		protected override void ProcessRecord()
		{
			...
			var Credential = new PasswordCredential(Target, UserName, Password);
	...
	public class RemoveStoredCredential : PSCmdlet
	{
		...
		protected override void ProcessRecord()
		{
			...
			try
			{
				...
				var Match = Vault.FindAllByResource(Target).SingleOrDefault();
